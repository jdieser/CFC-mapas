...
function clickHandler(e) {
	...
	// show wms url
	$('#url').text('WMS server: ' + (active_prov.properties.wms_url || '--'));
	
	adjustUlBottom();
}

function adjustUlBottom() {
	var url_height = parseInt($('#url').css('height')),
		github_height = parseInt($('#github').css('height')),
		bottom_value = url_height + github_height + 20;

	$('.sidebar-nav').css('bottom', bottom_value);
}
